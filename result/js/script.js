...
$(document).ready(function() {
	...
	function loadCSS (file, logo) {

		$("link[href*='style_']", "head").remove();
	...
	function setActiveTheme(theme) {
		var $trigger = $("a[data-theme^='"+theme+"']", "#themes"),
			switch_logo;
		if ($trigger.attr('data-logo'))	{
			switch_logo = $trigger.attr('data-logo');
		} else {
			switch_logo = '';
		}
		$trigger.css('border', '1px solid #ff9900');
		$('#logo img').attr('src', 'img/content/logo'+switch_logo+'.png');
	}


	if ($.cookie("theme"))
	{
		if ($.cookie("theme") == 'css/style.css') {
			$.cookie("theme", null);
		} else {
			setActiveTheme($.cookie("theme"));
			loadCSS($.cookie("theme"));
		}
	} else {
		console.log('test');
		setDefaultStyles();
		setActiveTheme('css/style.css');
	}


	$('a', '#themes').click(function() {
		loadCSS($(this).attr('data-theme'), $(this).attr('data-logo'));
